...
@Repository
@Transactional
public interface VendaCompraLojaVirtualReposiory extends JpaRepository<VendaCompraLojaVirtual, Long> {
    ...
    List<VendaCompraLojaVirtual> vendaPorCpfClienteLike(String nomepessoa);

    @Query(value = "select distinct(i.vendaCompraLojaVirtual) from ItemVendaLoja i "
            + " where i.vendaCompraLojaVirtual.excluido = false and upper(trim(i.vendaCompraLojaVirtual.pessoa.cpf)) = ?1")
    List<VendaCompraLojaVirtual> vendaPorCpfClienteIgual(String nomepessoa);
